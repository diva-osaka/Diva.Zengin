...
namespace Diva.Zengin.Formats
{
    ...
    /// 総合振込データの列マッピング定義
    /// </summary>
    public sealed class 総合振込DataMap : ClassMap<総合振込Data>
    {
        public 総合振込DataMap()
        {
            ...
            Map(m => m.EDI情報).Convert(args =>
                Get識別表示Value(args) == "Y" ? CharacterConverter.ConvertFromString(args.Row.GetField(11), true) : null);

            // 振込指定区分 - 列位置が動的
            Map(m => m.振込指定区分).Convert(args =>
            {
                // 振込指定区分のインデックスは識別表示によって変わる
                var index = Get識別表示Value(args) == "Y" ? 12 : 13;
        ...
        /// 識別表示の値を取得
        /// </summary>
        private static string Get識別表示Value(ConvertFromStringArgs args)
        {
